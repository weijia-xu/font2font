...
opt = {
   ...
   use_Lconst = 1,                    -- set to 0 to turn off constant loss
   which_model_netD = 'basic', -- selects model to use for netD
   which_model_netG = 'unet',  -- selects model to use for netG
   n_layers_D = 0,             -- only used if which_model_netD=='n_layers'
   L1_penalty = 100,           -- weight on L1 term in objective
   Lconst_penalty = 15,        -- weight on constant loss 
}
...
local criterionSQ = nn.MSECriterion()
---------------------------------------------------------------------------
optimStateG = {
   learningRate = opt.lr,
   beta1 = opt.beta1,
...
local fGx = function(x)
    ...
    -- unary loss
    local df_do_L1 = torch.zeros(fake_B:size())
    if opt.gpu>0 then 
    ...
    -- f-constancy term
    local df_dg_Lconst = torch.zeros(fake_B:size())
    if opt.gpu>0 then
      df_dg_Lconst = df_dg_Lconst:cuda();
    end
